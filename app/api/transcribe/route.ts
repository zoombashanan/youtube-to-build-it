...
import {
  fromAnthropicError,
  fromTranscriptError,
  unhandledTranscribeError,
} from "@/lib/transcribe-errors";
  ...
  try {
    ...
    if (!videoId) {
      return NextResponse.json(
        {
          error:
            "That does not look like a valid YouTube URL. Check the link and try again.",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { error: "Server error reading usage." },
        { status: 500 },
      );
    }
    if (count >= DAILY_CAP) {
